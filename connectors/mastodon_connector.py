...
class MastodonConnector(Connector):
    ...
    def initialize(self) -> bool:
        ...
        try:
            if not all([self.client_id, self.client_secret, self.access_token, self.api_base_url]):
                        ...
                        if img_response.status_code == 200:
                            ...
                            try:
                                ...
                                if metadata and metadata.get('viewer_count'):
                                    description_parts.append(f"👥 {metadata['viewer_count']:,} viewers")
                                if metadata and metadata.get('game_name'):
                                    description_parts.append(f"🎮 {metadata['game_name']}")
                                
                                description = " • ".join(description_parts) if description_parts else "Repository thumbnail"
