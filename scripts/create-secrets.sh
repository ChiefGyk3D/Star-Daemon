#!/bin/bash
# Star-Daemon - Secrets Management Setup Script
...
cat << "EOF"
╔═══════════════════════════════════════════════════════════╗
║         Star-Daemon - Secrets Setup Wizard               ║
...
# Settings
section_header "Star-Daemon Settings"
...
create_doppler_secrets() {
    ...
    echo ""
    prompt_with_default "Doppler Project Name" "star-daemon" "DOPPLER_PROJECT"
    ...
    if doppler projects get "$DOPPLER_PROJECT" &> /dev/null; then
        ...
        doppler projects create "$DOPPLER_PROJECT" --description "Star-Daemon secrets"
...
EOF
        ...
        aws secretsmanager create-secret \
            ...
            --description "Twitch API credentials for Star-Daemon" \
...
EOF
        ...
        aws secretsmanager create-secret \
            ...
            --description "YouTube API credentials for Star-Daemon" \
            --secret-string "$SECRET_VALUE" \
            --region "$AWS_REGION_CONFIG" 2>/dev/null || \
        aws secretsmanager update-secret \
...
EOF
        ...
        aws secretsmanager create-secret \
            ...
            --description "Kick API credentials for Star-Daemon" \
            --secret-string "$SECRET_VALUE" \
            --region "$AWS_REGION_CONFIG" 2>/dev/null || \
        aws secretsmanager update-secret \
...
EOF
        ...
        aws secretsmanager create-secret \
            ...
            --description "Mastodon API credentials for Star-Daemon" \
            --secret-string "$SECRET_VALUE" \
            --region "$AWS_REGION_CONFIG" 2>/dev/null || \
        aws secretsmanager update-secret \
...
EOF
        ...
        aws secretsmanager create-secret \
            ...
            --description "Bluesky API credentials for Star-Daemon" \
            --secret-string "$SECRET_VALUE" \
            --region "$AWS_REGION_CONFIG" 2>/dev/null || \
        aws secretsmanager update-secret \
    ...
    if [ "$DISCORD_ENABLE_POSTING" = "True" ]; then
        ...
        aws secretsmanager create-secret \
            ...
            --description "Discord webhooks for Star-Daemon" \
            --secret-string "$SECRET_VALUE" \
            --region "$AWS_REGION_CONFIG" 2>/dev/null || \
        aws secretsmanager update-secret \
    ...
    if [ "$MATRIX_ENABLE_POSTING" = "True" ]; then
        ...
        if [ "$SECRET_VALUE" != "{}" ]; then
            aws secretsmanager create-secret \
                ...
                --description "Matrix credentials for Star-Daemon" \
                --secret-string "$SECRET_VALUE" \
                --region "$AWS_REGION_CONFIG" 2>/dev/null || \
            aws secretsmanager update-secret \
...
EOF
        ...
        aws secretsmanager create-secret \
            ...
            --description "LLM API keys for Star-Daemon" \
            --secret-string "$SECRET_VALUE" \
            --region "$AWS_REGION_CONFIG" 2>/dev/null || \
        aws secretsmanager update-secret \
...
create_vault_secrets() {
    ...
    # Create secrets
    prompt_with_default "Vault secret path prefix" "secret/star-daemon" "SECRET_PATH"
...
# doppler run --project $DOPPLER_PROJECT_NAME --config $DOPPLER_CONFIG_NAME -- python star-daemon.py
...
echo "   python3 star-daemon.py --test"
echo ""
echo "2. ${GREEN}Run Star-Daemon:${NC}"

case $MANAGER in
    doppler)
        echo "   ${CYAN}Option 1:${NC} doppler run --project $DOPPLER_PROJECT_NAME --config $DOPPLER_CONFIG_NAME -- python3 star-daemon.py"
        if [ -n "$DOPPLER_SERVICE_TOKEN" ]; then
            echo "   ${CYAN}Option 2:${NC} python3 star-daemon.py (uses DOPPLER_TOKEN from .env)"
        fi
        ;;
    aws|vault|env)
        echo "   python3 star-daemon.py"
        ;;
esac
